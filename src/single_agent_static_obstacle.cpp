...
int main(int argc, char **argv)
{
    ...
    double v_x_max=0.1;//向前的线速度0.2m/s
    double v_y_max=0.1;//向前的线速度0.2m/s
    double v_x_t=0.0;//向前的线速度0.2m/s
    double v_y_t=0.0;//向前的线速度0.2m/s
    double v_x_k = 2.0;
    double v_y_k = 2.0;
    double obstacle_x_set = 1.0;
    double obstacle_y_set = 0.0;
...
//                 v_y_t = abs(v_y_d + v_y_k * delta_y) > v_x_max ? v_x_max : (v_y_d + v_y_k * delta_y);
                ...
                double F1_y_temp = v_y_d + v_y_k * delta_y - lambda_y * B_y;
                double F1_x,F1_y;
...
//                 ROS_INFO_STREAM();
                ...
                double F2_x = (lambda_x + B_x * v_x_t + Bright) < 0.0000000000000000000001 ? 0.0 : (lambda_x + B_x * v_x_t + Bright);
                double F2_y = (lambda_y + B_y * v_y_t + Bright) < 0.0000000000000000000001 ? 0.0 : (lambda_y + B_y * v_y_t + Bright);
                ROS_INFO_STREAM("F1_x Vel= "<< F1_x << "\tF1_y Vel= "<< F1_y<<"\tF2_x = "<< F2_x << "\tF2_y = "<< F2_y);
                
                
                //参数更新率
                double lambda_rate_x = (F2_x - lambda_x) / (10 * epsilon);
                double lambda_rate_y = (F2_y - lambda_y) / (10 * epsilon);
                ROS_INFO_STREAM("lambda_rate_x = "<< lambda_rate_x << "\tlambda_rate_y = "<< lambda_rate_y);
...
//                 ROS_INFO_STREAM();
                ...
                ROS_INFO_STREAM("lambda_x = "<< lambda_x<<"\tlambda_y = "<< lambda_y);
//                 ROS_INFO_STREAM();
                t_last = t_run;
